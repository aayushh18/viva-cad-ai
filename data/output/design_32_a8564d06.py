import FreeCAD as App
import FreeCADGui as Gui
import Part

doc = App.newDocument("TextCAD")
result = None

box_0 = doc.addObject("Part::Box", "Box_0")
box_0.Length = 100.0
box_0.Width = 100.0
box_0.Height = 100.0
doc.recompute()

if result is None:
    result = box_0
else:
    fusion_0 = doc.addObject("Part::Fuse", "Fuse_0")
    fusion_0.Base = result
    fusion_0.Tool = box_0
    doc.recompute()
    result = fusion_0

try:
    shape = result.Shape
    fillet_shape = shape.makeFillet(10.0, shape.Edges)
    fillet_obj = doc.addObject("Part::Feature", "Fillet_1")
    fillet_obj.Shape = fillet_shape
    result = fillet_obj
    doc.recompute()
except Exception as e:
    print(f"Fillet failed: {e}")

doc.recompute()
try:
    if Gui.ActiveDocument and Gui.ActiveDocument.ActiveView:
        Gui.ActiveDocument.ActiveView.viewIsometric()
        Gui.ActiveDocument.ActiveView.fitAll()
except Exception as e:
    print(f"View scaling failed: {e}")

App.ActiveDocument.saveAs("/home/aayush/Downloads/viva-cad-ai/data/output/design_32_a8564d06.FCStd")
