...
try:
    ...
    try:
        print('Applying cylinder (c1)')
        c1 = doc.addObject('Part::Cylinder', 'c1')
        c1.Radius = 5
        c1.Height = 10
        current_obj = c1
        doc.recompute()
    except Exception as e:
        print(f'ERROR applying cylinder: {e}')
    try:
        if current_obj: print(f'Applying fillet on {current_obj.Name}')
        if current_obj:
            edges = current_obj.Shape.Edges
            if edges:
                f1 = doc.addObject('Part::Fillet', 'f1')
                f1.Base = current_obj
                f1.Edges = [(i+1, 5, 5) for i in range(len(edges))]
    ...
    try:
        if current_obj: print(f'Applying chamfer on {current_obj.Name}')
        if current_obj:
            edges = current_obj.Shape.Edges
            if edges:
                ch1 = doc.addObject('Part::Chamfer', 'ch1')
                ch1.Base = current_obj
                ch1.Edges = [(i+1, 2, 2) for i in range(len(edges))]
                set_visible(current_obj.Name, False)
                current_obj = ch1
        doc.recompute()
    except Exception as e:
        print(f'ERROR applying chamfer: {e}')
    try:
    ...
    doc.saveAs('/home/aayush/Downloads/viva-cad-ai/data/output/design_16_ef5945a1.FCStd')
    ...
    if visible_objs:
        Part.export(visible_objs, '/home/aayush/Downloads/viva-cad-ai/data/output/design_16_ef5945a1.step')
        __objs_to_export__ = []
        ...
        Mesh.export(__objs_to_export__, '/home/aayush/Downloads/viva-cad-ai/data/output/design_16_ef5945a1.stl')
